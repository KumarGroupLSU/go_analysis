...
PROGRAM proton_hop
    ...
    !   ----------------------------------------------- Infos/properties
    REAL(dp), ALLOCATABLE           :: atm_mat(:,:,:), is_mat(:,:,:), H3O_mat(:,:,:), HOPP(:,:,:)
    CHARACTER(LEN=3), ALLOCATABLE   :: atm_name(:,:)
    INTEGER, ALLOCATABLE            :: nb_is(:), nb_max_H3O(:)
    INTEGER                         :: nb_max_is, nb_o, nb_h3o, hopp_c, nb_max_HOPP
    ...
    IF ( nb_h3o .NE. 0) THEN
        ...
        ! B ----------------------------------------------- Allocate and calculate closest distance between IS and H3O atom
        start = OMP_get_wtime()

        ALLOCATE(H3O_mat(12,nb_o,nb_step))
        ALLOCATE(nb_max_H3O(nb_step))
        H3O_mat(:,:,:) = 0.0_dp
        nb_max_H3O(:) = 0
        !$OMP PARALLEL DO DEFAULT(NONE) SHARED(atm_mat, box, is_mat, nb_is, nb_step, nb_atm, H3O_mat, nb_max_H3O)&
        !$OMP PRIVATE(s, i, j, o)&
        !$OMP PRIVATE(ISaX_disp_norm)
        DO s = 1, nb_step
            o = 0
            DO i = 1, nb_atm
                IF (atm_mat(3,i,s) .EQ. 35) THEN ! H3O
                    DO j = 1, nb_is(s)
                    ...
                    END DO
                    o = o + 1
                    H3O_mat(1:12,o,s) = atm_mat(1:12,i,s)
                END IF
            END DO
            nb_max_H3O(s) = COUNT( H3O_mat(1,:,s) .NE. 0, DIM=1 )
        END DO
        !$OMP END PARALLEL DO

        finish = OMP_get_wtime()
        PRINT'(A40,F14.2,A20)', "Proximity IS and H3O+:", finish-start, "seconds elapsed"

        ! B ----------------------------------------------- Calculate the p-hop
        start = OMP_get_wtime()

        ALLOCATE(HOPP(12,MAXVAL(nb_max_H3O),nb_step))
        HOPP(:,:,:) = 0.0_dp
        hopp_c = 0
        nb_max_HOPP = 0
        
        W:DO s = 1, nb_step
            IF ( nb_max_H3O(s) .EQ. 0 ) THEN ! Skip when no proton at all in step s
                CYCLE W
            END IF
            X:DO i = 1, nb_max_H3O(s) ! Cycle all protons in step s
                IF (H3O_mat(1,i,s) .EQ. 0 ) THEN ! Skip empty
                    CYCLE X
                END IF
                IF ( s .EQ. 1 ) THEN ! Init
                    ...
                    HOPP(12,i,s) = hopp_c
                ELSE
                    IF ( nb_max_H3O(s-1) .EQ. 0 ) THEN ! Init when no proton at all in step s-1
                        HOPP(2,i,s) = H3O_mat(1,i,s)
                        HOPP(11,i,s) = H3O_mat(7,i,s)*H3O_mat(8,i,s)
                        hopp_c = hopp_c + 1
                        HOPP(12,i,s) = hopp_c
                        CYCLE X
                    END IF
                Y:DO j = 1, nb_max_H3O(s-1) ! Cycle all protons in step s-1
                        IF (HOPP(2,j,s-1)  .EQ. 0 ) THEN  ! Skip empty
                            CYCLE Y
                        END IF
                        IF ( H3O_mat(1,i,s) .EQ. HOPP(2,j,s-1) ) THEN ! If index ID s is same as s-1, update HOPP(s)
                            DO k = 1,10
                        ...
                        ELSE
                            ...
                            CALL sb_dist(H3O_mat(4:6,i,s),H3O_mat(4:6,j,s-1),box,norm_ij=prev_new_norm) ! calculate distance between index s and index s-1
                            ...
                            IF ( prev_new_norm .LT. 4.00) THEN ! If less than 3 and not in the memory, jump
                                ...
                                ( H3O_mat(1,i,s)  .NE. HOPP(6,j,s-1) ) ) THEN
                                    ...
                                    HOPP(12,i,s) = HOPP(12,j,s-1)
                                    CYCLE X
                                ELSE IF ( H3O_mat(1,i,s) .EQ. HOPP(3,j,s-1) ) THEN
                                    HOPP(1,i,s) = HOPP(1,j,s-1) - 1
                                    HOPP(2,i,s) = H3O_mat(1,i,s)
                                    HOPP(11,i,s) = H3O_mat(7,i,s)*H3O_mat(8,i,s)
                                    DO k = 3,9
                                    ...
                                    HOPP(12,i,s) = HOPP(12,j,s-1)
                                    CYCLE X
                                ELSE IF ( H3O_mat(1,i,s) .EQ. HOPP(4,j,s-1) ) THEN
                                    ...
                                    HOPP(12,i,s) = HOPP(12,j,s-1)
                                    CYCLE X
                                ELSE IF ( H3O_mat(1,i,s) .EQ. HOPP(5,j,s-1) ) THEN
                                    ...
                                    HOPP(12,i,s) = HOPP(12,j,s-1)
                                    CYCLE X
                                ELSE IF ( H3O_mat(1,i,s) .EQ. HOPP(6,j,s-1) ) THEN
